        ...
        <div className=' text-white min-h-[90vh] px-10 max-w-[1280px] mx-auto items-center grid '>
            ...
            <div className=" grid gap-20 mb-20 grid-cols-3">
                {teams.map((member, index) => (
                  <Link className='group hover:bg-[#5454D4] h-[320px] overflow-hidden bg-gray-600 hover:scale-110 transition-all duration-300' to={`${member.name.replace(/\s+/g, '_')}`} key={index}>
                  <div className=''>
                      <img className='object-fit relative h-[200px] top-[1px] grayscale group-hover:grayscale-0 transition-all duration-300 mx-auto' src={member.image} alt={member.name} />
                  </div>
